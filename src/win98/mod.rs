...
impl Win98Window {
    ...
    fn draw_window_controls(f: &mut Frame, area: Rect) {
        ...
        let controls = Paragraph::new(" _ □ ×")
            .style(Style::default().fg(Color::Black).bg(colors::BUTTON_FACE));
    ...
    fn render(self, area: Rect, buf: &mut Buffer) {
        ...
        for (i, cluster) in self.clusters.iter().enumerate() {
            ...
            if let Some(cell) = buf.cell_mut((area.x + x, area.y + y)) {
                cell.set_symbol("█").set_fg(color).set_bg(Color::Black);
            }
        }
    }
}
...
pub fn render_win98_app(app: &App, frame: &mut Frame) {
    ...
    let bg_block = Block::default().style(Style::default().bg(colors::DESKTOP_TEAL));
